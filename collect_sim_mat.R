...
#####################################################
## this script generate similarity matrices for significant functional terms for EBI datasets

library(cola)
library(GetoptLong)

setwd("/omics/groups/OE0246/internal/guz/simplifyGO_test")
...
for(nm in names(diff_gene_lt2)) {
	g = diff_gene_lt2[[nm]]
	bsub_chunk(name = qq("enrichment_@{nm}"), hour = 1, memory = 10, variables = c("nm", "g"),
	{	
		library(GetoptLong)
		res = list()
		...
		saveRDS(res, file = qq("/omics/groups/OE0246/internal/guz/simplifyGO_test/rds/enrichment/@{nm}_enrichment.rds"))
	})
}
...
for(i in seq_along(all_nm)) {
	nm = all_nm[i]
	bsub_chunk(name = qq("generate_sim_mat_@{i}"), hour = 1, memory = 5, variables = c("nm", "all_nm", "i"),
	{ 
		setwd("/omics/groups/OE0246/internal/guz/simplifyGO_test")
...
### save all similarity matrices into rds files
